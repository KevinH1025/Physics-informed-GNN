...
from .physics import dc_gain_log10_formula, ugbw_log10_estimate
...
class TowerGENConv(BaseGNN):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._build_input_options(node_feature_dim, kwargs)
        ...
        self._build_backbone(
            hidden_dim, backbone_layers, state_tower_layers,
            sensitivity_tower_layers, genconv_num_layers, norm_type, dropout,
            edge_dim, use_virtual_node, use_attention_pooling,
            vn_learn_temperature, vn_gate_broadcast, vn_mode, vn_num_heads,
            vn_head_dim, loop_attention_config, backbone_jk_config, kwargs,
        )
        self._build_state_tower(
            hidden_dim, state_tower_layers, backbone_layers,
            genconv_num_layers, norm_type, dropout, edge_dim,
            state_tower_jk_config,
        )

        # Prediction heads input dim
        mlp_input_dim = hidden_dim + node_feature_dim if skip_connection else hidden_dim

        self._build_state_heads(
            hidden_dim, mlp_input_dim, norm_type, predict_currents,
            use_device_pooling_current, state_tower_layers,
            voltage_head_config, current_head_config, kwargs,
        )
        self._build_autograd_or_iv_model(mlp_input_dim, ss_head_config)
        self._build_sensitivity_tower(
            hidden_dim, mlp_input_dim, backbone_layers,
            sensitivity_tower_layers, genconv_num_layers, norm_type, dropout,
            edge_dim, ss_head_config, sensitivity_tower_jk_config, kwargs,
        )
        self._build_gm_id_head(hidden_dim, mlp_input_dim, kwargs)
        self._build_ac_head(hidden_dim, mlp_input_dim, norm_type, kwargs)
        self._build_subcircuit_dag(hidden_dim, kwargs)
        ...
        self._build_dc_gain_head(hidden_dim, mlp_input_dim, kwargs)
    ...
    def _build_input_options(self, node_feature_dim, kwargs):
    ...
    def _build_backbone(self, hidden_dim, backbone_layers, state_tower_layers,
                        sensitivity_tower_layers, genconv_num_layers, norm_type,
                        dropout, edge_dim, use_virtual_node,
                        use_attention_pooling, vn_learn_temperature,
                        vn_gate_broadcast, vn_mode, vn_num_heads, vn_head_dim,
                        loop_attention_config, backbone_jk_config, kwargs):
        # --- Backbone layers ---
    ...
    def _build_state_tower(self, hidden_dim, state_tower_layers,
                           backbone_layers, genconv_num_layers, norm_type,
                           dropout, edge_dim, state_tower_jk_config):
        # --- State Tower (V/I) ---
    ...
    def _build_state_heads(self, hidden_dim, mlp_input_dim, norm_type,
                           predict_currents, use_device_pooling_current,
                           state_tower_layers, voltage_head_config,
                           current_head_config, kwargs):
        # Voltage head
    ...
    def _build_autograd_or_iv_model(self, mlp_input_dim, ss_head_config):
    ...
    def _build_sensitivity_tower(self, hidden_dim, mlp_input_dim,
                                 backbone_layers, sensitivity_tower_layers,
                                 genconv_num_layers, norm_type, dropout,
                                 edge_dim, ss_head_config,
                                 sensitivity_tower_jk_config, kwargs):
    ...
    def _build_gm_id_head(self, hidden_dim, mlp_input_dim, kwargs):
    ...
    def _build_ac_head(self, hidden_dim, mlp_input_dim, norm_type, kwargs):
            ...
            if ac_output_dim == 0:
                ...
                if self.ac_readout in ('cross_attn', 'mha_pool'):
                    ...
                    self.register_buffer('_ac_mosfet_indices', torch.tensor(
                        SIGNAL_PATH_MOSFET_INDICES, dtype=torch.long))
    ...
    def _build_subcircuit_dag(self, hidden_dim, kwargs):
        ...
        if self.use_subcircuit_dag:
            ...
            self.register_buffer('_sc_assignment', build_subcircuit_assignment(sc_groups))
            # Build topological order from DAG edges
            self._sc_topo_order = build_dag_topo_order(sc_edges, self._sc_num_groups)
            # Attention pooler
    ...
    def _build_dc_gain_head(self, hidden_dim, mlp_input_dim, kwargs):
    ...
    def forward(self, data) -> Dict[str, torch.Tensor]:
        ...
        x_in, x = self._forward_input(data)
        ...
        edge_attr = self._forward_edge_attr(data)
        ...
        _loop_attn, _loop_ei, _loop_dtm, _node_loop_ei = self._forward_loop_setup(data)
        ...
        _sc_embs, _sc_embs_live = self._forward_subcircuit_dag(data, backbone_hidden)

        tower_input, _gm_id_pred = self._forward_gm_id(data, backbone_repr, backbone_hidden)

        state_outputs, state_hidden = self._forward_state_tower(
            data, tower_input, edge_attr, batch_vec, num_graphs, vn_emb,
            backbone_outputs, _loop_attn, _loop_ei, _loop_dtm, _node_loop_ei,
        )
        ...
        result = self._forward_state_predictions(data, state_repr, state_outputs, _gm_id_pred)

        mosfet_info, offsets, num_mosfets, num_graphs = self._mosfet_offsets(data)

        self._forward_vov_vth(result, state_repr, mosfet_info, offsets)

        sens_repr = self._forward_sensitivity_tower(
            data, result, x_in, edge_attr, batch_vec, num_graphs, vn_emb,
            tower_input, state_hidden, state_repr, backbone_outputs,
            mosfet_info, offsets, num_mosfets,
            _loop_attn, _loop_ei, _loop_dtm, _node_loop_ei,
        )

        self._forward_iv_model(result, mosfet_info, offsets, state_repr)

        self._forward_dc_gain(
            data, result, num_graphs, mosfet_info, offsets, sens_repr,
            backbone_mha_outputs, vn_emb, batch_vec, backbone_repr,
            backbone_hidden,
        )

        self._forward_ac(
            data, result, num_graphs, mosfet_info, offsets, sens_repr,
            backbone_mha_outputs, batch_vec, vn_emb, state_repr,
            _sc_embs, _sc_embs_live, backbone_hidden,
        )
    ...
    def _forward_edge_attr(self, data):
    ...
    def _forward_loop_setup(self, data):
        # Loop attention data
        _loop_attn = None
        _loop_ei = None
        _loop_dtm = None
        ...
        return _loop_attn, _loop_ei, _loop_dtm, _node_loop_ei

    def _forward_subcircuit_dag(self, data, backbone_hidden):
        ...
        return _sc_embs, _sc_embs_live

    def _forward_gm_id(self, data, backbone_repr, backbone_hidden):
        ...
        return tower_input, _gm_id_pred

    def _forward_state_tower(self, data, tower_input, edge_attr, batch_vec,
                             num_graphs, vn_emb, backbone_outputs,
                             _loop_attn, _loop_ei, _loop_dtm, _node_loop_ei):
        # --- State Tower ---
        ...
        return state_outputs, state_hidden

    def _forward_state_predictions(self, data, state_repr, state_outputs, _gm_id_pred):
        # State predictions
        result = {}
    ...
    def _mosfet_offsets(self, data):
        ...
        return mosfet_info, offsets, num_mosfets, num_graphs

    def _forward_vov_vth(self, result, state_repr, mosfet_info, offsets):
    ...
    def _forward_sensitivity_tower(self, data, result, x_in, edge_attr,
                                   batch_vec, num_graphs, vn_emb, tower_input,
                                   state_hidden, state_repr, backbone_outputs,
                                   mosfet_info, offsets, num_mosfets,
                                   _loop_attn, _loop_ei, _loop_dtm,
                                   _node_loop_ei):
        sens_repr = None
        # --- Sensitivity Tower ---
        if self.has_sensitivity_tower:
    ...
    def _forward_iv_model(self, result, mosfet_info, offsets, state_repr):
    ...
    def _forward_dc_gain(self, data, result, num_graphs, mosfet_info, offsets,
                         sens_repr, backbone_mha_outputs, vn_emb, batch_vec,
                         backbone_repr, backbone_hidden):
        # DC gain head
        if self.predict_dc_gain:
            if self.dc_gain_mode == 'physics':
                ...
                formula_feats, dc_gain_est_dB = dc_gain_log10_formula(
                    gm_key, gds_key, self._dc_rout1_formula,
                    getattr(self, '_dc_gain_openloop', False),
                    self._dc_r_in, self._dc_r_f,
                )
    ...
    def _forward_ac(self, data, result, num_graphs, mosfet_info, offsets,
                    sens_repr, backbone_mha_outputs, batch_vec, vn_emb,
                    state_repr, _sc_embs, _sc_embs_live, backbone_hidden):
        ...
        if self.predict_ac:
            if self.ac_readout in ('cross_attn', 'mha_pool') and len(backbone_mha_outputs) > 0:
                ...
                log10_ugbw_est = ugbw_log10_estimate(
                    z_gm_key, self.ss_gm_mean_buf, self.ss_gm_std_buf,
                    Cc_norm, self._ac_cc_log10_min, self._ac_cc_log10_range,
                    getattr(self, '_ac_openloop', False),
                    self._dc_r_in, self._dc_r_f,
                )
