...
class IVEmbedder(nn.Module):
    ...
    def __init__(self, run_dir: str | Path, lut_path: Optional[str | Path] = None,
        ...
        if self.use_vbs:
            ...
            n_id = np.transpose(n_id, (0, 1, 2, 4, 3))
            ...
            if not np.allclose(n_vbs_mag, p_vbs_mag):
                raise ValueError('NMOS and PMOS |Vbs| axes differ — '
                                 'expected mirrored grids')
            order_n = np.argsort(n_vbs_mag)
            order_p = np.argsort(p_vbs_mag)
            n_log = n_log[:, :, order_n, :, :]
            p_log = p_log[:, :, order_p, :, :]
            vbs_mag = n_vbs_mag[order_n].astype(np.float32)

            lut = np.stack([n_log, p_log], axis=0)  # [2, W, L, Vbs, Vgs, Vds]
            ...
            n_id = np.transpose(n_id, (0, 1, 3, 2))
            p_id = np.transpose(p_id, (0, 1, 3, 2))
            n_log = np.log10(np.maximum(np.abs(n_id), self.LOG_FLOOR)).astype(np.float32)
            p_log = np.log10(np.maximum(np.abs(p_id), self.LOG_FLOOR)).astype(np.float32)
            lut = np.stack([n_log, p_log], axis=0)  # [2, W, L, Vgs, Vds]
            self.register_buffer('lut', torch.from_numpy(lut))
            self._nVbs = 0
