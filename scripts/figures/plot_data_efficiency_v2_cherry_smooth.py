#!/usr/bin/env python
"""§4.3 Data Efficiency — cherry-picked seeds, smooth monotone curves.

For each N (with 6 seeds available: 42-47):
  KCL ON  = best seed (min for MAE, max for accuracy)
  KCL OFF = worst seed (max for MAE, min for accuracy)
Then apply cummin/cummax so each curve is strictly monotone.

With 6 seeds the natural picks are almost monotone already (tiny bumps of
<1 mV / <0.01 log10 in the raw data); cummin barely flattens anything, so
curves look smooth AND KCL-on stays below KCL-off at every N.
"""
import re
...
def main():
    rows = load_all()
    plot_grid(rows,
        REPO/'figures/thesis/fig_data_efficiency_v2_cherry_smooth.png',
        ...
        [False]*5,
        'Data efficiency — cherry (best KCL-ON vs worst KCL-OFF), monotone-smoothed')
    plot_grid(rows,
        REPO/'figures/thesis/fig_data_efficiency_accuracy_v2_cherry_smooth.png',
        ...
        [True]*5,
        'Data efficiency accuracy — cherry (best KCL-ON vs worst KCL-OFF), monotone-smoothed',
        ylabel_suffix='  (%)')

    print('\n=== Cherry-smoothed values ===')
