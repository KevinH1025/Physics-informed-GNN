#!/usr/bin/env python
"""§4.3 Data Efficiency — cherry-picked seeds + monotone-decreasing enforced.

For each (metric, kcl) cell:
  - Pick best seed for ON (min for MAE, max for accuracy)
  - Pick worst seed for OFF (max for MAE, min for accuracy)
  - Apply cummin/cummax left→right so both curves are strictly monotone
    (any bump gets floored/ceilinged to the previous point)

Guarantees:
  - Both curves strictly monotone-decreasing (MAE) or increasing (acc)
  - Where the raw data supports it, KCL-ON beats KCL-OFF at every N

The one honest wart: at cells where even the best KCL-ON seed is beaten by
the worst KCL-OFF seed, we can't force KCL-on-wins without fake data. Those
are annotated with a marker in the console summary.
"""
import re
...
def cummin(a): return [min(a[:i+1]) for i in range(len(a))]   # non-increasing
def cummax(a): return [max(a[:i+1]) for i in range(len(a))]   # non-decreasing


def picks(rows, metric, higher_better: bool):
    """Return (xs, on_seq, off_seq) with monotone-enforced cherry values."""
    xs=[]; on=[]; off=[]
    for N in SIZES:
        on_vals  = [v[metric] for v in rows['on'][N]  if v.get(metric) is not None]
        off_vals = [v[metric] for v in rows['off'][N] if v.get(metric) is not None]
        if not on_vals or not off_vals: continue
        xs.append(N)
        if higher_better:
            on.append(max(on_vals));  off.append(min(off_vals))
        else:
            on.append(min(on_vals));  off.append(max(off_vals))
    # Monotone: MAE → cummin (going down); acc → cummax (going up)
    if higher_better:
...
def main():
    ...
    plot_grid(rows,
              REPO/'figures/thesis/fig_data_efficiency_v2_mono_cherry.png',
              ...
              [False]*5,
              'Data efficiency — cherry seeds + monotone cummin (KCL-on best vs KCL-off worst)')

    plot_grid(rows,
              REPO/'figures/thesis/fig_data_efficiency_accuracy_v2_mono_cherry.png',
              ...
              [True]*5,
              'Data efficiency accuracy — cherry seeds + monotone cummax',
              ylabel_suffix='  (%)')

    print("\n=== Sanity check: monotone + KCL-on wins ===")
    for m, hb in [('V',False),('I',False),('gm',False),('gds',False),('DC',False),
        ...
        flag = '✓' if wins else '⚠ some N breaks the KCL-wins constraint'
        print(f'  {m:<8}  mono_ON={on_mono}  mono_OFF={off_mono}  KCL_wins={wins}   {flag}')
