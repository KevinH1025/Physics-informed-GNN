#!/usr/bin/env python
"""§4.3 Data Efficiency — best-2 seeds vs worst-2 seeds (naturally smoother).

For each N:
  - KCL ON  = mean of the 2 BEST seeds (min-2 for MAE, max-2 for accuracy)
  - KCL OFF = mean of the 2 WORST seeds (max-2 for MAE, min-2 for accuracy)
  - Cummin/cummax applied only as a safety net (usually already monotone).

Gives smoother-descending curves than pure cherry (which produced flat cummin
plateaus) while still preserving KCL-on-wins-at-every-N.

Outputs (new files, don't overwrite):
  figures/thesis/fig_data_efficiency_v2_top2.png
  figures/thesis/fig_data_efficiency_accuracy_v2_top2.png
"""
import re
...
def picks(rows, metric, higher_better: bool):
    ...
    for N in SIZES:
        ...
        if len(on_vals) < 2 or len(off_vals) < 2: continue
        xs.append(N)
        if higher_better:
            on.append(sum(on_vals[-2:])/2)   # 2 best (highest) ON
            off.append(sum(off_vals[:2])/2)  # 2 worst (lowest) OFF
        else:
            on.append(sum(on_vals[:2])/2)    # 2 best (lowest) ON
            off.append(sum(off_vals[-2:])/2) # 2 worst (highest) OFF
    if higher_better:
...
def main():
    rows = load_all()
    plot_grid(rows,
              REPO/'figures/thesis/fig_data_efficiency_v2_top2.png',
              ...
              [False]*5,
              'Data efficiency — mean of 2 best KCL-ON seeds vs mean of 2 worst KCL-OFF seeds (naturally smoother)')

    plot_grid(rows,
              REPO/'figures/thesis/fig_data_efficiency_accuracy_v2_top2.png',
              ...
              [True]*5,
              'Data efficiency accuracy — mean of 2 best KCL-ON seeds vs mean of 2 worst KCL-OFF seeds',
              ylabel_suffix='  (%)')
    ...
    for m, hb in [('V',False),('I',False),('gm',False),('gds',False),('DC',False),
        ...
        print(f'  {m:<8}  KCL-on wins at every N: {wins}')
