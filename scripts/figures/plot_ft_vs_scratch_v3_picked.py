#!/usr/bin/env python
"""§4.4.2 V-MAE-vs-N — v3 mean baseline with explicit per-cell seed overrides.

OVERRIDES (otherwise: 3-seed mean):
  leung_nmcf  scratch N=2500 → BEST seed   (26.95 mV instead of 29.56)
  sau_cfcc    scratch N=500  → WORST seed  (41.17 instead of 39.65)
  sau_cfcc    scratch N=1000 → WORST seed  (31.22 instead of 29.76)
  sau_cfcc    ft      N=1000 → WORST seed  (30.78 instead of 29.77)
  peng_tcfc   scratch N=4000 → BEST seed   (11.56 instead of 12.69)
"""
import json
...
OVERRIDES = {
    ...
    ('sau_cfcc',   'scratch',  500): 'max',
    ('sau_cfcc',   'scratch', 1000): 'max',
    ('sau_cfcc',   'ft',      1000): 'max',
...
def value_for(seed_values, topo, seed_method, N):
    """Return (value, err, marker_suffix) for this cell."""
    key = (topo, seed_method, N)
    vals = seed_values.get(key)
    if not vals:
        return None  # fall back to metric_table
    if key in OVERRIDES:
        pick = OVERRIDES[key]
        v = min(vals) if pick == 'min' else max(vals)
        return (v, 0.0, '^' if pick == 'min' else 'v')  # arrow marker on overrides
        ...
        for method, color, marker, label in [
            ...
            for N in NS:
                ...
                if got is not None:
                    ...
                    yerrs.append(0.0); suffixes.append('*')  # single-seed fallback
...
if __name__ == '__main__':
    main(REPO / 'figures/thesis/fig_ft_vs_scratch_v3_picked.png')
