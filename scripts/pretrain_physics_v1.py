...
class PhysicsPretrainModel(nn.Module):
    ...
    def forward(self, data):
        ...
        # default-mode virtual node path (init_embedding, per-layer
        # broadcast, post-layer VN state update). It matches encode()
        # bitwise only when the VN is disabled or vn mode == 'mha'; with vn
        # mode 'default' this loop silently skips the VN while encode()
        # applies it. Kept as is to preserve behavior; not switched to
        # backbone.encode().
...
def main():
    ...
    pbar = make_progress_bar(epochs, len(train_loader), desc='PINN')
    ...
    for epoch in range(epochs):
        ...
        # Scheduler and best checkpoint both track val_v: V loss only, a more
        # stable signal (kcl scale fluctuates with warmup). Backbone-only
        # state dict.
        best_val, msg = scheduler_step_and_save_best(
            val_v, best_val, epoch, model.backbone, cfg, out_dir, msg,
            scheduler=sch, extra_ckpt={'val_v': val_v, 'val_kcl': val_kcl},
        )
